...
function SWEP:CreateCustomizeHUD()
    local barsize = ssm(160)
    local airgap = ssm(16)
    local smallgap = ssm(2)
    local linesize = ssm(1)
    local buttonsize = ssm(32)
    ...
    loadpresets:SetSize((barsize - ssm(2)) / 2, ssm(14))
    loadpresets:SetText("")
    ...
    loadpresets.Paint = function(spaa, w, h)
        ...
        surface.SetTextPos(smallgap, ssm(1))
        surface.SetFont("ArcCW_12")
        surface.DrawText(txt)
    end

    local savepresets = vgui.Create("DButton", ArcCW.InvHUD)
    savepresets:SetSize((barsize - ssm(2)) / 2, ssm(14))
    savepresets:SetText("")
    savepresets:SetPos(scrw - (barsize / 2) + ssm(1) - airgap, airgap)
    ...
    local attcatsy = scrh - ssm(64) - airgap
    ...
    triviabox:SetText("")
    triviabox:SetSize(barsize, scrh - ssm(64) - (3 * airgap))
    ...
    statbox:SetText("")
    statbox:SetSize(barsize, scrh - ssm(64) - (3 * airgap))
    ...
    attmenu:SetText("")
    attmenu:SetSize(barsize + ssm(12), attmenuh)
    ...
    attslidebox:SetSize(barsize, ssm(20))
    attslidebox:SetPos(scrw - barsize - airgap, scrh - ssm(64) - (1 * airgap))
    ...
    attslider:SetPos(ssm(4), ssm(12))
    attslider:SetSize(barsize - ssm(4 * 2), ssm(4))

    attslider.Paint = function(span, w, h)
        surface.SetDrawColor(fg_col)
        surface.DrawRect(0, h / 2, w, ssm(1))
    end
    ...
    attslider.Knob.Paint = function(span, w, h)
        ...
        if span:IsHovered() or attslider:GetDragging() then
            surface.SetDrawColor(fg_col)
            surface.DrawRect((w - ssm(2)) / 2, 0, ssm(2), h)
        else
            surface.SetDrawColor(fg_col)
            surface.DrawRect((w - ssm(1)) / 2, 0, ssm(1), h)
        end
    ...
    atttogglebtn:SetSize(barsize * 0.25, ssm(15))
    atttogglebtn:SetPos(scrw - barsize - airgap + barsize * 0.75, scrh - ssm(64) - (1 * airgap))
    ...
    atttrivia:SetSize(barsize, scrh - ssm(116))
    ...
    local function atttrivia_do(att, slot)
        ...
        local triv_attname = vgui.Create("DLabel", atttrivia)
            triv_attname:SetSize(barsize, ssm(16))
        ...
        triv_desc:SetSize(barsize, ssm(8) * (table.Count(desctext) + 1))
        triv_desc:SetText("")
        ...
        if (pros and #pros or 0) > 0 then
            ...
            triv_pros:SetSize(barsize, ssm(10))
            triv_pros:SetText("")
            triv_pros:Dock(TOP)
            ...
            for _, i in pairs(pros) do
                ...
                triv_pro:SetSize(barsize, ssm(10))
                triv_pro:SetText("")
                triv_pro:Dock(TOP)
        ...
        if (cons and #cons or 0) > 0 then
            ...
            triv_cons:SetSize(barsize, ssm(10))
            triv_cons:SetText("")
            triv_cons:Dock(TOP)
            ...
            for _, i in pairs(cons) do
                ...
                triv_con:SetSize(barsize, ssm(10))
                triv_con:SetText("")
                triv_con:Dock(TOP)
        ...
        if #neutrals > 0 then
            ...
            triv_neutrals:SetSize(barsize, ssm(10))
            ...
            for _, i in pairs(neutrals) do
                ...
                triv_neutral:SetSize(barsize, ssm(10))
        ...
        local function attcatb_regen(span)
            ...
            if self.Attachments[span.AttIndex].Installed and catttbl and catttbl.ToggleStats then
                if attslidebox:IsVisible() then
                    atttogglebtn:SetPos(scrw - barsize - airgap + barsize * 0.75, scrh - ssm(40) - (1 * airgap))
                else
                    atttogglebtn:SetPos(scrw - barsize - airgap + barsize * 0.75, scrh - ssm(64) - (1 * airgap))
                end
                atttogglebtn:Show()
            ...
            for _, att in pairs(atts) do
                ...
                attbtn:SetSize(barsize + ssm(12), ssm(14))
                attbtn:SetText("")
                attbtn:Dock( TOP )
                ...
                attbtn.Paint = function(spaa, w, h)
                    ...
                    surface.SetTextColor(Bfg_col)
                    surface.SetTextPos((h * 1.5) + smallgap, ssm(1))
                    surface.SetFont("ArcCW_12")

                    DrawTextRot(spaa, txt, h * 1.5, 0, (h * 1.5) + smallgap, ssm(1), w - (h * 1.5))
            ...
            local specsize = ca * (ssm(14) + smallgap)

            attmenu:SetSize(barsize + ssm(12), math.min(specsize, attmenuh))
        end
    ...
    local trivia = {
        function() return ArcCW.TryTranslation(self:GetBuff_Override("Override_Trivia_Class") or self.Trivia_Class) or "Unknown" end,
        function() return year or "Unknown" end,
        function() return ArcCW.TryTranslation(self:GetBuff_Override("Override_Trivia_Mechanism") or self.Trivia_Mechanism or "Unknown") end,
        function() return ArcCW.TryTranslation(self:GetBuff_Override("Override_Trivia_Calibre") or self.Trivia_Calibre or "Unknown") end,
        function() return language.GetPhrase(self.Primary.Ammo or self.PrintName) end,
        function() return ArcCW.TryTranslation(self:GetBuff_Override("Override_Trivia_Country") or self.Trivia_Country or "Unknown") end,
        function() return ArcCW.TryTranslation(self:GetBuff_Override("Override_Trivia_Manufacturer") or self.Trivia_Manufacturer or "Unknown") end,
        function() return self:GetCapacity() end,
        function() return self:GetBuff("AccuracyMOA") .. " MOA" end,
        function() return (self:GetBuff("ShootVol")) .. "dB" end,
        function() return math.Truncate(self.Recoil * 41.4 * self:GetBuff_Mult("Mult_Recoil"), 1) .. " lb-fps" end,
        function() return math.Round(self:GetBuff("Penetration"), 1) .. "mm" end,
    }

    local trivia2 = {
        function() return translate("trivia.class") end,
        function() return translate("trivia.year") end,
        function() return translate("trivia.mechanism") end,
        function() return translate("trivia.calibre") end,
        function() return translate("trivia.ammo") end,
        function() return translate("trivia.country") end,
        function() return translate("trivia.manufacturer") end,
        function() return translate("trivia.clipsize") end,
        function() return translate("trivia.precision") end,
        function() return translate("trivia.noise") end,
        function() return translate("trivia.recoil") end,
        function() return translate("trivia.penetration") end,
    }
    ...
    if !self.ManualAction and !self:GetBuff_Override("Override_ManualAction") then
        ...
        table.insert(trivia2, function()
            local rpm = math.Round(60 / self:GetFiringDelay())
            return translate("trivia.firerate")
        end)
    end
    ...
    for _, i in pairs(trivia) do
        ...
        triv_misc:SetSize(barsize, ssm(14))
        triv_misc:Dock(TOP)
        triv_misc:SetText("")
    ...
    for _, i in pairs(trivia2) do
        if !i then continue end
        local triv_misc = vgui.Create("DLabel", triviabox)
        triv_misc:SetSize(barsize, ssm(12))
        triv_misc:Dock(TOP)
        triv_misc:SetText("")
        triv_misc:DockMargin( 0, 0, 0, 0 )
        triv_misc.Paint = function(span, w, h)
    ...
    triv_desc:SetSize(barsize, ssm(12) * (table.Count(adesctext) + 1))
    triv_desc:SetText("")
    triv_desc:Dock(TOP)
    triv_desc.Paint = function(span, w, h)
        local y = ssm(8)
    ...
    if !self.ShootEntity and !self.PrimaryBash and !self.Throwing and !self.NoRangeGraph then
        ...
        rangegraph:SetSize(barsize, ssm(64))
        rangegraph:SetText("")
        rangegraph:Dock(TOP)
        rangegraph.Paint = function(span, w, h)
            ...
            local gw, gh = w - (2 * sidegap), h - smallgap - ssm(6)
            ...
            if mingr != 0 then
                surface.DrawLine(gx + startx, gy+ssm(2) + starty, gx + startx, gy-ssm(2) + starty)
            end
            -- long and into the void
            surface.DrawLine(gx + endx, gy + endy, gx + gw, gy + endy)
            surface.DrawLine(gx + endx, gy+ssm(2) + endy, gx + endx, gy-ssm(2) + endy)

            -- start dmg
            surface.SetTextColor(fg_col)
            surface.SetFont("ArcCW_6")
            surface.SetTextPos(0, gy + starty - ssm(7) - 1)
            ...
            surface.SetTextPos(gx + gw - dtw, gy + endy - ssm(7) - 1)
            ...
            if mousex > gx and mousex < (gx + gw) and
                ...
                if mousex < math.max(twmr, twmb) + ssm(2) then
                    alignleft = false
                end
                ...
                surface.SetFont("ArcCW_6")
                if alignleft then
                    surface.SetTextPos(mousex - ssm(2) - twmr, shy)
                else
                    surface.SetTextPos(mousex + ssm(2), shy)
                end
                ...
                surface.SetFont("ArcCW_6")
                if alignleft then
                    surface.SetTextPos(mousex - ssm(2) - twmb, ssm(2) + gy)
                else
                    surface.SetTextPos(mousex + ssm(2), ssm(2) + gy)
                end
    ...
    regenStatList = function()
        ...
        for _, i in pairs(statList) do
            ...
            stat_panel:SetSize(barsize, ssm(10))
            stat_panel:Dock(TOP)
            stat_panel:SetText("")
            stat_panel:DockMargin( 0, ssm(1), 0, ssm(1) )
            ...
            stat_title:SetSize(barsize * 0.5, ssm(10))
            stat_title:SetText("")
            ...
            stat_orig:SetSize(barsize * 0.25, ssm(10))
            stat_orig:SetText("")
            stat_orig:Dock(LEFT)
            ...
            stat_cur:SetSize(barsize * 0.25, ssm(10))
            stat_cur:SetText("")
            stat_cur:Dock(LEFT)
    ...
    if !self.Throwing and !self.PrimaryBash and !self.ShootEntity then
        ...
        togglestat:SetSize((barsize - ssm(2)) / 2, ssm(14))
        togglestat:SetText("")
        togglestat:SetPos(scrw - barsize - airgap - ssm(1) - (barsize / 2), airgap)
    ...
    if engine.ActiveGamemode() == "terrortown" then
        ...
        if GetRoundState() == ROUND_ACTIVE and (LocalPlayer():GetTraitor() or LocalPlayer():GetDetective() or (TTT2 and LocalPlayer().IsShopper and LocalPlayer():IsShopper())) then
            ...
            buymenu:SetSize((barsize - ssm(2)) / 2, ssm(14))
            buymenu:SetText("")
            buymenu:SetPos(scrw - barsize - airgap - ssm(1) - (barsize / 2), airgap + gap)
            gap = gap + airgap
        ...
        radiomenu:SetSize((barsize - ssm(2)) / 2, ssm(14))
        radiomenu:SetText("")
        radiomenu:SetPos(scrw - barsize - airgap - ssm(1) - (barsize / 2), airgap + gap)
